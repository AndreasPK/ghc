...
unboxArg arg
  ...
  = do dflags <- getDynFlags
       ...
       return (Var prim_arg,
              \ body -> Case (mkWildCase arg arg_ty intPrimTy
                                       [(DataAlt falseDataCon defaultFreq,[],mkIntLit dflags 0),
                                        (DataAlt trueDataCon defaultFreq, [],mkIntLit dflags 1)])
                                        -- In increasing tag order!
                             prim_arg
                             (exprType body)
                             [(DEFAULT defaultFreq,[],body)]) --TODO: Freq?
  ...
  = ASSERT2(isUnliftedType data_con_arg_ty1, pprType arg_ty)
    ...
    do case_bndr <- newSysLocalDs arg_ty
       ...
       return (Var prim_arg,
               \ body -> Case arg case_bndr (exprType body) [(DataAlt data_con alwaysFreq,[prim_arg],body)]
              )
  ...
  = do case_bndr <- newSysLocalDs arg_ty
       ...
       return (Var arr_cts_var,
               \ body -> Case arg case_bndr (exprType body) [(DataAlt data_con alwaysFreq,vars,body)]
              )
...
mk_alt return_result (Nothing, wrap_result)
  = do -- The ccall returns ()
       ...
       let
             ...
             the_alt      = (DataAlt (tupleDataCon Unboxed 1) alwaysFreq, [state_id], the_rhs) --TODO: Freq?
  ...
  = -- The ccall returns a non-() value
    ...
    do { result_id <- newSysLocalDs prim_res_ty
       ...
       ; let the_rhs = return_result (Var state_id)
             ...
             the_alt      = (DataAlt (tupleDataCon Unboxed 2) alwaysFreq, [state_id, result_id], the_rhs) --TODO:Freq?
       ; return (ccall_res_ty, the_alt) }
  ...
  = do { dflags <- getDynFlags
       ; let marshal_bool e
               = mkWildCase e intPrimTy boolTy
                   [ (DEFAULT                    defaultFreq,[],Var trueDataConId )
                   , (LitAlt (mkLitInt dflags 0) defaultFreq,[],Var falseDataConId)]
