...
coreToStgExpr df (Case scrut bndr _ alts) = do
  ...
  where
    ...
    vars_alt (con, binders, rhs)
      | DataAlt c w <- con, c == unboxedUnitDataCon
      = -- This case is a bit smelly.
        -- See Note [Nullary unboxed tuple] in Type.hs
        -- where a nullary tuple is mapped to (State# World#)
        ASSERT( null binders )
        do { rhs2 <- coreToStgExpr df rhs
           ; return (DEFAULT w, [], rhs2)  }
      | otherwise
      = let     -- Remove type variables
            binders' = filterStgBinders binders
        in
        extendVarEnvCts [(b, LambdaBound) | b <- binders'] $ do
        rhs2 <- coreToStgExpr df rhs
...
mkStgAltType bndr alts
  ...
  where
   ...
   look_for_better_tycon
        | ((DataAlt con _, _, _) : _) <- data_alts = --TODOW Check
