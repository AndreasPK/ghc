...
instance Eq (DeBruijn CoreAlt) where
  D env1 a1 == D env2 a2 = go a1 a2 where
    go (DEFAULT _, _, rhs1) (DEFAULT _, _, rhs2)
        = D env1 rhs1 == D env2 rhs2
    go (LitAlt lit1 _, _, rhs1) (LitAlt lit2 _, _, rhs2)
        = lit1 == lit2 && D env1 rhs1 == D env2 rhs2
    go (DataAlt dc1 _, bs1, rhs1) (DataAlt dc2 _, bs2, rhs2)
        = dc1 == dc2 &&
...
lkA env (DEFAULT _,    _, rhs)  = am_deflt >.> lkG (D env rhs)
lkA env (LitAlt lit _, _, rhs)  = am_lit >.> lookupTM lit >=> lkG (D env rhs)
lkA env (DataAlt dc _, bs, rhs) = am_data >.> lkDNamed dc
                                        >=> lkG (D (extendCMEs env bs) rhs)

xtA :: CmEnv -> CoreAlt -> XT a -> AltMap a -> AltMap a
xtA env (DEFAULT _, _, rhs)    f m =
    m { am_deflt = am_deflt m |> xtG (D env rhs) f }
xtA env (LitAlt l _, _, rhs)   f m =
    m { am_lit   = am_lit m   |> alterTM l |>> xtG (D env rhs) f }
xtA env (DataAlt d _, bs, rhs) f m =
