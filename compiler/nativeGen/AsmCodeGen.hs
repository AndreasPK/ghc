...
--
noAllocMoreStack :: Int -> NatCmmDecl statics instr -> UniqSM (NatCmmDecl statics instr, [(BlockId,BlockId)])
...
nativeCodeGen' :: (Outputable statics, Outputable instr, Outputable jumpDest, Instruction instr)
               => DynFlags
               -> Module -> ModLocation
...
cmmNativeGen dflags this_mod modLoc ncgImpl us fileIds dbgMap cmm count
 = do
        ...
        let cfgStackAloced = foldl' (\m (from,to) -> addImmediateSuccessor from to m ) nativeCfgWeights stack_updt_blks

        let cfgRegUpdates = (concatMap Linear.ra_fixupList raStats)

        let postRegCFG = addNodeBetween cfgStackAloced cfgRegUpdates
        ...
        dumpIfSet_dyn dflags
                ...
                ( text "stack:" <+> ppr stack_updt_blks $$
                  text "linearAlloc:" <+> ppr cfgRegUpdates )

        ---- shortcut branches
        let -- shorted :: [NatCmmDecl statics instr]
            (shorted, postShortCFG)     =
...
build_mapping ncgImpl (CmmProc info lbl live (ListGraph (head:blocks)))
  ...
  where
    ...
    mapping = mapFromList shortcut_blocks --foldl' add mapEmpty shortcut_blocks
    --add m (id,dest) = mapAdd id dest m
