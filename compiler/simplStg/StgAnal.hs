...
data Strictness = Evaluated | Unknown

type StrictEnv = IdEnv Strictness

evalArg :: StrictEnv -> StgArg -> (StrictEnv, [Id])
evalArg env (StgLitArg _) = (env, [])
evalArg env (StgVarArg args) =
    (env, [])
where

analExpr :: StrictEnv -> StgExpr -> [Id]
analExpr (StgApp _f _args) = []

analExpr (StgLit _lit) = []

analExpr (StgConApp _con _args _tys) = []
