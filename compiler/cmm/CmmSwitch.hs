...
import BasicTypes (BranchWeight, combinedFreqs, moreLikely,
                   ...
                   representsNumWeight, mkCompilerWeight
                   )
import Outputable
import DynFlags
import Maybes
...
type LabelInfo = (Label, EdgeWeight)

liLbl :: LabelInfo -> Label
liLbl = fst

liWeight :: LabelInfo -> EdgeWeight
liWeight = snd
...
mkSwitchTargets signed range@(lo,hi) mbdef ids
  ...
  where
    ids' = --fmap (second normalize) $
            dropDefault $ restrict ids
    mbdef' | defaultNeeded = --fmap (second normalize)
                             mbdef :: Maybe LabelInfo
           | otherwise     = Nothing
    ...
    -- Normalization of the branch weights:
    -- Replace unknown weights with totalWeight/n.
    -- We use intermediate doubles to avoid overflow
    -- on intermediate results.
    -- mbweight :: Int
    -- mbweight
    --   | Just mb <- mbdef = fromMaybe 0 $ (getWeight . snd) mb
    --   | otherwise         = 0
    -- altWeights = mapMaybe (getWeight . snd) $ M.elems ids
    -- totalWeight = foldl' (+) (0 :: Double) (map fromIntegral $ mbweight : altWeights)
    -- altCount = fromIntegral $ (maybe 0 (const 1) mbdef') + M.size ids'
    -- normalize :: BranchWeight -> EdgeWeight
    -- normalize w
    --   | representsNumWeight w
    --   = fromIntegral $
    --       (1000 * fromIntegral $ expectJust "normalizeSwitchWeights" (getWeight w))
    --       / totalWeight
    --   | otherwise = fromIntegral $ totalWeight / fromIntegral altCount
...
switchTargestsToWeightedList (SwitchTargets _ _ mbdef branches)
    = let (labels, weights) = unzip (maybeToList mbdef ++ M.elems branches)
          ...
          floatWeights = map fromIntegral $ weights :: [Double]
...
createSwitchPlan _ (SwitchTargets _signed _range (Just defInfo) m)
    ...
    = IfEqual x li
        ...
        (moreLikelyThan (liWeight li) (liWeight defInfo))
...
createSwitchPlan _ (SwitchTargets _signed (lo,hi) Nothing m)
    ...
    , hi - lo == 1
    = IfEqual x1 li1 (Unconditionally li2) (moreLikelyThan f1 f2)
...
createSwitchPlan _ (SwitchTargets _signed _range (Just def@(_, fdef)) m)
    ...
    = IfEqual x1 li1
        (IfEqual x2 li2 (Unconditionally def) (moreLikelyThan f2 fdef))
        (moreLikelyThan f1 (f2 + fdef))
...
-- | Convert two weights to if/then else likely value.
moreLikelyThan :: EdgeWeight -> EdgeWeight -> Maybe Bool
moreLikelyThan n1 n2
  | n1 < n2 = Just False
  | n1 == n2 = Just True
  | otherwise = Nothing
...
findSingleValues (Unconditionally l, (i, Unconditionally l2) : (i', Unconditionally l3) : xs)
  ...
  = findSingleValues
      (IfEqual i l2
        (Unconditionally l) (moreLikelyThan (liWeight l2) (liWeight l))
      , xs)
...
buildTree byWeight signed sl
  ...
  , sp_likely = likely
   ...
   where
    ...
    likely = if byWeight
      then moreLikelyThan (planWeight left) (planWeight right)
      else Nothing
...
divideBalanced (p,xs) = --pprTrace "divideTree" (
  ...
  where
    rweight = sum . map (planWeight . snd) $ xs
    lweight = planWeight p :: EdgeWeight

    --((p,leftSplit),m,(pr,rightSplit))

    weightSpan :: EdgeWeight -> EdgeWeight -> [(Integer,SwitchPlan)] -> [(Integer,SwitchPlan)] -> ([(Integer,SwitchPlan)], [(Integer,SwitchPlan)])
    weightSpan _ _ lxs [] = (lxs,[])
    weightSpan lw rw lxs ((i,sp):rxs)
      | lw >= rw = (lxs,((i,sp):rxs))
      | lw < rw = weightSpan (lw + (planWeight sp)) (rw - (planWeight sp)) ((i,sp):lxs) rxs
