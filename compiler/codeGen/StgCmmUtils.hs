...
import BasicTypes (BranchWeight, EdgeWeight(..), representsNumWeight, getWeight)
import ForeignCall
import IdInfo
import Type
import TyCon
...
mk_discrete_switch signed tag_expr branches mb_deflt range
  = mkSwitch tag_expr $
      mkSwitchTargets signed range mbdef' ids
  where
    ids = (M.fromList $ map (\(i,e,f)-> (i,(e,normalize f))) branches)
    mbdef' = fmap (second normalize) mb_deflt :: Maybe LabelInfo
    ...
    mbweight :: Double
    mbweight
      | Just mb <- mb_deflt = fromIntegral $ fromMaybe 0 $ (getWeight . snd) mb
      | otherwise         = 0
    altWeights = mapMaybe (getWeight . thdOf3) branches
    totalWeight = foldl' (+) mbweight (map fromIntegral altWeights)
    altCount = (maybe 0 (const 1) mb_deflt) + length branches
    ...
    normalize w
      ...
      = ceiling $
          ...
          / totalWeight
      | otherwise = ceiling $ totalWeight / (fromIntegral altCount)
--------------
...
emitCmmLitSwitch scrut  branches (deflt,dfreq) = do
    ...
    let signed = case head branches of
                    (LitNumber nt _ _, _, _) -> litNumIsSigned nt
                    _ -> False
